...
def parse_args():
    parser = argparse.ArgumentParser(description="Train anomaly detection models for medical imaging")
    parser.add_argument('--config', type=str, default='Config.yaml', help='Path to config file')
    parser.add_argument('--model_type', type=str, help='Override model type from config (vae or gan)')
    parser.add_argument('--epochs', type=int, help='Override max epochs from config')
    parser.add_argument('--batch_size', type=int, help='Override batch size from config')
    parser.add_argument('--run_id', type=str, default=None, help='Run identifier for logging')
...
def train_vae(model, train_loader, val_loader, optimizer, scheduler, config, writer, model_dir):
    ...
    for epoch in range(config['max_epochs']):
        ...
        print(f"Epoch {epoch}/{config['max_epochs']} complete. "
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(f"Model saved at epoch {epoch} with validation loss {best_val_loss:.6f}")
        else:
        ...
        if epoch % config['save_interval'] == 0:
            torch.save({
                ...
                'optimizer_state_dict': optimizer.state_dict(),
                'loss': avg_val_loss,
            }, model_dir / f"checkpoint_epoch_{epoch}.pt")
...
def train_gan(model, train_loader, val_loader, config, writer, model_dir):
    ...
    for epoch in range(config['max_epochs']):
        ...
        for batch_idx, (data, _) in enumerate(train_loader):
            ...
            feature_matching_loss = l2_loss(fake_features, real_features.detach())
            
            # Generator loss for fooling discriminator
            g_loss = adversarial_loss(fake_pred, real_target) + feature_matching_loss
            g_loss.backward()
            optimizer_G.step()
        ...
        with torch.no_grad():
            for data, _ in val_loader:
                ...
                val_loss += nn.MSELoss()(reconstructed, data).item()
...
def main():
    ...
    if config['model_type'].lower() == 'vae':
        ...
        model = train_vae(model, train_loader, val_loader, optimizer, scheduler, config, writer, model_dir)
    
    elif config['model_type'].lower() == 'gan':
        model = train_gan(model, train_loader, val_loader, config, writer, model_dir)
